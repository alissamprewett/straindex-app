...
// Try a couple of public Overpass mirrors in order — the main instance
// occasionally 4xx's cloud-hosting IP ranges under load; a mirror often
// succeeds when that happens.
...
async function runQueryAt(url, lat, lon, radiusMeters) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[geodispensaries] ${url} returned ${res.status}: ${bodyText.slice(0, 500)}`);
      return { ok: false, reason: `OpenStreetMap lookup returned an error (status ${res.status}).` };
    }
    const data = await res.json();
    return { ok: true, elements: data.elements || [] };
  } catch (err) {
    console.error(`[geodispensaries] ${url} fetch failed:`, err);
...
// Tries each known Overpass mirror in turn, returning the first success.
async function runQuery(lat, lon, radiusMeters) {
  let lastFailure = { ok: false, reason: 'Could not reach the dispensary lookup service.' };
  for (const url of OVERPASS_ENDPOINTS) {
    const outcome = await runQueryAt(url, lat, lon, radiusMeters);
