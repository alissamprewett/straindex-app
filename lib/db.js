...
// ---------- Check-ins ----------
async function createCheckin({ user_id, strain_id, method, rating, note, effects, photo, tasting_notes, pairing_food, pairing_entertainment, pairing_activity, is_private, brand }) {
  const rs = await client.execute({
    sql: `INSERT INTO checkins (user_id, strain_id, method, rating, note, effects, photo, tasting_notes, pairing_food, pairing_entertainment, pairing_activity, is_private, brand) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *`,
    args: [user_id, strain_id, method || '', rating || 0, note || '', JSON.stringify(effects || []), photo || null, tasting_notes || '', pairing_food || '', pairing_entertainment || '', pairing_activity || '', is_private ? 1 : 0, brand || ''],
  });
...
async function updateCheckin(id, { method, rating, note, effects, photo, tasting_notes, pairing_food, pairing_entertainment, pairing_activity, is_private, brand }) {
  ...
  await client.execute({
    sql: `UPDATE checkins SET method=?, rating=?, note=?, effects=?, photo=?, tasting_notes=?, pairing_food=?, pairing_entertainment=?, pairing_activity=?, is_private=?, brand=? WHERE id=?`,
    args: [merged.method || '', merged.rating || 0, merged.note || '', JSON.stringify(merged.effects || []), merged.photo || null, merged.tasting_notes || '', merged.pairing_food || '', merged.pairing_entertainment || '', merged.pairing_activity || '', merged.is_private ? 1 : 0, merged.brand || '', id],
  });
  const idx = cache.checkins.findIndex(c => c.id === id);
