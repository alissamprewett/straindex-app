...
// ---------- Strains ----------
function listStrains({ q, type, rarity, effect, limit = 60 } = {}) {
  assertReady();
  ...
  if (effect && effect !== 'All') arr = arr.filter(s => Array.isArray(s.effects) && s.effects.includes(effect));
  arr.sort((a, b) => a.name.localeCompare(b.name));
  return arr.slice(0, limit);
}
function countStrains({ q, type, rarity, effect } = {}) {
  assertReady();
  let arr = [...cache.strains.values()];
  if (q) { const needle = q.toLowerCase(); arr = arr.filter(s => s.name.toLowerCase().includes(needle)); }
  if (type && type !== 'All') arr = arr.filter(s => s.type === type);
