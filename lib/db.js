...
function matchesFilters(s, { q, type, rarity, effect, thc, terpene, ailment, breeder, verified }) {
  if (q) {
    ...
    const inAka = Array.isArray(s.aka) && s.aka.some(a => a.toLowerCase().includes(needle));
...
async function insertStrain(s) {
  ...
  await client.execute({
    sql: `INSERT OR REPLACE INTO strains (id,name,type,lean,rarity,thc,cbd,terps,effects,flavor,icon,breeder,ailments,parents)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    args: [s.id, s.name, s.type, s.lean || '', s.rarity, s.thc || '', s.cbd || '', terps, effects, s.flavor || '', s.icon || '🌿', s.breeder || null, ailments, parents],
  });
