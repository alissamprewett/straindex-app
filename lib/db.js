...
// ---------- Collection (derived from check-ins — one "card" per check-in) ----------
function getCollection() {
  const rows = db.prepare('SELECT strain_id, COUNT(*) AS copies FROM checkins GROUP BY strain_id').all();
  return rows.map(r => ({ strain: getStrain(r.strain_id), copies: r.copies })).filter(r => r.strain);
}
function getUniqueOwnedCount() {
  return db.prepare('SELECT COUNT(DISTINCT strain_id) AS c FROM checkins').get().c;
}
function getTotalDupes() {
  const rows = db.prepare('SELECT COUNT(*) AS c FROM checkins GROUP BY strain_id').all();
  return rows.reduce((sum, r) => sum + Math.max(0, r.c - 1), 0);
}
function getMostCheckedInStrains(limit = 4) {
  const rows = db.prepare('SELECT strain_id, COUNT(*) AS c FROM checkins GROUP BY strain_id ORDER BY c DESC LIMIT ?').all(limit);
  return rows.map(r => ({ strain: getStrain(r.strain_id), count: r.c })).filter(r => r.strain);
}

// ---------- Trades (demo trade log — see /trade route for the honest caveat) ----------
function createTrade({ friend_name, gave_strain_id, got_strain_id }) {
  db.prepare('INSERT INTO trades (friend_name, gave_strain_id, got_strain_id) VALUES (?,?,?)').run(friend_name, gave_strain_id, got_strain_id);
}
function listTrades(limit = 50) {
  return db.prepare('SELECT * FROM trades ORDER BY id DESC LIMIT ?').all(limit);
}
function countTrades() {
  return db.prepare('SELECT COUNT(*) AS c FROM trades').get().c;
}

// ---------- Dispensary follows ----------
function isFollowingDispensary(id) {
  return !!db.prepare('SELECT 1 FROM dispensary_follows WHERE dispensary_id = ?').get(id);
}
function toggleFollowDispensary(id) {
  if (isFollowingDispensary(id)) db.prepare('DELETE FROM dispensary_follows WHERE dispensary_id = ?').run(id);
  else db.prepare('INSERT INTO dispensary_follows (dispensary_id) VALUES (?)').run(id);
  return isFollowingDispensary(id);
}
function anyDispensaryFollowed() {
  return db.prepare('SELECT COUNT(*) AS c FROM dispensary_follows').get().c > 0;
}

// ---------- Event RSVPs ----------
function isRsvped(id) {
  return !!db.prepare('SELECT 1 FROM event_rsvps WHERE event_id = ?').get(id);
}
function toggleRsvp(id) {
  if (isRsvped(id)) db.prepare('DELETE FROM event_rsvps WHERE event_id = ?').run(id);
  else db.prepare('INSERT INTO event_rsvps (event_id) VALUES (?)').run(id);
  return isRsvped(id);
}
function anyRsvped() {
  return db.prepare('SELECT COUNT(*) AS c FROM event_rsvps').get().c > 0;
}

// ---------- Shop cart (demo — a running count, not a real checkout) ----------
function addToCart(itemId) {
  db.prepare('INSERT INTO cart_items (item_id) VALUES (?)').run(itemId);
}
function getCartCount() {
  return db.prepare('SELECT COUNT(*) AS c FROM cart_items').get().c;
}
