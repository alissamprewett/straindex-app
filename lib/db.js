...
async function createCheckin({ user_id, strain_id, method, rating, note, effects, photo, tasting_notes, pairings, is_private, brand }) {
  const rs = await client.execute({
    sql: `INSERT INTO checkins (user_id, strain_id, method, rating, note, effects, photo, tasting_notes, pairings, is_private, brand) VALUES (?,?,?,?,?,?,?,?,?,?,?) RETURNING *`,
    args: [user_id, strain_id, method || '', rating || 0, note || '', JSON.stringify(effects || []), photo || null, tasting_notes || '', JSON.stringify(normalizePairings(pairings)), is_private ? 1 : 0, brand || ''],
  });
...
async function deleteUserAccount(userId) {
  ...
  await client.execute({ sql: 'DELETE FROM feedback WHERE user_id = ?', args: [userId] });
  await client.execute({ sql: 'DELETE FROM strain_submissions WHERE user_id = ?', args: [userId] });
...
function getYearInReview(userId, year) {
  ...
  checkins.forEach(c => {
    ...
    if (s) typeCounts[s.type] = (typeCounts[s.type] || 0) + 1;
    strainCheckinCounts[c.strain_id] = (strainCheckinCounts[c.strain_id] || 0) + 1;
    uniqueStrainIds.add(c.strain_id);
...
module.exports = {
  ...
  getStrainRatingStats, getSimilarStrains, getUserInsights, getCheckinStreak, getYearInReview,
