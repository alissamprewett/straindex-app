...
function rowToStrain(row) {
  return { ...row, terps: JSON.parse(row.terps), effects: JSON.parse(row.effects), ailments: row.ailments ? JSON.parse(row.ailments) : [], parents: row.parents ? JSON.parse(row.parents) : [] };
}
...
async function init() {
  ...
  // Migration: two more optional pairing types -- setting/location and
  // mood/occasion -- added alongside the original three so the check-in
  // form's pairing picker has more than three options to choose from.
  if (!checkinPairingCols.includes('pairing_setting')) {
    await client.execute('ALTER TABLE checkins ADD COLUMN pairing_setting TEXT');
  }
  if (!checkinPairingCols.includes('pairing_occasion')) {
    await client.execute('ALTER TABLE checkins ADD COLUMN pairing_occasion TEXT');
  }
...
function matchesFilters(s, { q, type, rarity, effect, thc, terpene, ailment, breeder, verified }) {
  if (q) {
    ...
    const inAka = !!s.aka && s.aka.toLowerCase().includes(needle);
    if (!inName && !inFlavor && !inAka) return false;
  }
  ...
  if (verified && verified !== 'All' && verificationTier(s) !== verified) return false;
  return true;
}
...
async function insertStrain(s) {
  ...
  await client.execute({
    sql: `INSERT OR REPLACE INTO strains (id,name,type,lean,rarity,thc,cbd,terps,effects,flavor,icon,breeder,ailments,parents,aka)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    args: [s.id, s.name, s.type, s.lean || '', s.rarity, s.thc || '', s.cbd || '', terps, effects, s.flavor || '', s.icon || '🌿', s.breeder || null, ailments, parents, aka || null],
  });
...
// ---------- Check-ins ----------
async function createCheckin({ user_id, strain_id, method, rating, note, effects, photo, tasting_notes, pairing_food, pairing_entertainment, pairing_activity, pairing_setting, pairing_occasion, is_private, brand }) {
  const rs = await client.execute({
    sql: `INSERT INTO checkins (user_id, strain_id, method, rating, note, effects, photo, tasting_notes, pairing_food, pairing_entertainment, pairing_activity, pairing_setting, pairing_occasion, is_private, brand) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *`,
    args: [user_id, strain_id, method || '', rating || 0, note || '', JSON.stringify(effects || []), photo || null, tasting_notes || '', pairing_food || '', pairing_entertainment || '', pairing_activity || '', pairing_setting || '', pairing_occasion || '', is_private ? 1 : 0, brand || ''],
  });
...
async function updateCheckin(id, { method, rating, note, effects, photo, tasting_notes, pairing_food, pairing_entertainment, pairing_activity, pairing_setting, pairing_occasion, is_private, brand }) {
  ...
  const merged = {
    ...
    pairing_setting: pairing_setting !== undefined ? pairing_setting : current.pairing_setting,
    pairing_occasion: pairing_occasion !== undefined ? pairing_occasion : current.pairing_occasion,
    is_private: is_private !== undefined ? (is_private ? 1 : 0) : current.is_private,
    brand: brand !== undefined ? brand : current.brand,
  };
  await client.execute({
    sql: `UPDATE checkins SET method=?, rating=?, note=?, effects=?, photo=?, tasting_notes=?, pairing_food=?, pairing_entertainment=?, pairing_activity=?, pairing_setting=?, pairing_occasion=?, is_private=?, brand=? WHERE id=?`,
    args: [merged.method || '', merged.rating || 0, merged.note || '', JSON.stringify(merged.effects || []), merged.photo || null, merged.tasting_notes || '', merged.pairing_food || '', merged.pairing_entertainment || '', merged.pairing_activity || '', merged.pairing_setting || '', merged.pairing_occasion || '', merged.is_private ? 1 : 0, merged.brand || '', id],
  });
  const idx = cache.checkins.findIndex(c => c.id === id);
...
async function createUserFromGoogle({ username, birth_date, email, google_id, first_name, last_name }) {
  ...
  const rs = await client.execute({
    ...
    args: [username, hash, salt, birth_date, email || null, google_id, first_name, last_name],
  });
  const row = rowToObject(rs);
...
async function createUser({ username, password, birth_date, email, first_name, last_name }) {
  if (getUserByUsername(username)) {
    throw new Error('That username is already taken.');
  }
  ...
  const rs = await client.execute({
    sql: 'INSERT INTO users (username, password_hash, password_salt, birth_date, email, first_name, last_name) VALUES (?,?,?,?,?,?,?) RETURNING *',
    args: [username, hash, salt, birth_date, email || null, first_name, last_name],
  });
  const row = rowToObject(rs);
  cache.users.set(row.id, row);
  cache.usernameIndex.set(row.username.toLowerCase(), row.id);
...
async function deleteUserAccount(userId) {
  ...
  await client.execute({ sql: 'DELETE FROM checkin_kudos WHERE user_id = ?', args: [userId] });
  await client.execute({ sql: 'DELETE FROM comment_likes WHERE user_id = ?', args: [userId] });
  await client.execute({ sql: 'DELETE FROM checkin_comments WHERE user_id = ?', args: [userId] });
  await client.execute({ sql: 'DELETE FROM messages WHERE sender_id = ? OR recipient_id = ?', args: [userId, userId] });
  ...
  cache.checkinKudos = cache.checkinKudos.filter(k => k.user_id !== userId);
  cache.commentLikes = cache.commentLikes.filter(l => l.user_id !== userId);
  cache.checkinComments = cache.checkinComments.filter(c => c.user_id !== userId);
...
module.exports = {
  init,
  listStrains, countStrains, matchesFilters, getStrain, insertStrain, deleteStrain, nextStrainId,
  ...
  updateUsername, updatePassword, updateEmail, updateName, adminUpdateUser,
