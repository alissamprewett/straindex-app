...
function matchesFilters(s, { q, type, rarity, effect, thc, terpene }) {
  if (q) {
...
function listStrains({ q, type, rarity, effect, thc, terpene, limit = 60 } = {}) {
  assertReady();
  let arr = [...cache.strains.values()].filter(s => matchesFilters(s, { q, type, rarity, effect, thc, terpene }));
  arr.sort((a, b) => a.name.localeCompare(b.name));
  return arr.slice(0, limit);
}
function countStrains({ q, type, rarity, effect, thc, terpene } = {}) {
  assertReady();
  return [...cache.strains.values()].filter(s => matchesFilters(s, { q, type, rarity, effect, thc, terpene })).length;
}
...
async function createRecipe({ title, time, icon, source = 'community', author, user_id, desc, ingredients, steps, dosing, category = 'Baked Goods', status = 'approved', kudos = 0 }) {
  const rs = await client.execute({
    sql: `INSERT INTO recipes (title,time,icon,source,author,user_id,kudos,desc,ingredients,steps,dosing,category,status)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *`,
    args: [
