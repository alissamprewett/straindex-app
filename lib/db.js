...
// ---------- Check-ins ----------
async function createCheckin({ user_id, strain_id, method, rating, note, effects, photo, tasting_notes, pairing_food, pairing_entertainment, pairing_activity }) {
  const rs = await client.execute({
    sql: `INSERT INTO checkins (user_id, strain_id, method, rating, note, effects, photo, tasting_notes, pairing_food, pairing_entertainment, pairing_activity) VALUES (?,?,?,?,?,?,?,?,?,?,?) RETURNING *`,
    args: [user_id, strain_id, method || '', rating || 0, note || '', JSON.stringify(effects || []), photo || null, tasting_notes || '', pairing_food || '', pairing_entertainment || '', pairing_activity || ''],
  });
...
async function updateCheckin(id, { method, rating, note, effects, photo, tasting_notes, pairing_food, pairing_entertainment, pairing_activity }) {
  ...
  await client.execute({
    sql: `UPDATE checkins SET method=?, rating=?, note=?, effects=?, photo=?, tasting_notes=?, pairing_food=?, pairing_entertainment=?, pairing_activity=? WHERE id=?`,
    args: [merged.method || '', merged.rating || 0, merged.note || '', JSON.stringify(merged.effects || []), merged.photo || null, merged.tasting_notes || '', merged.pairing_food || '', merged.pairing_entertainment || '', merged.pairing_activity || '', id],
  });
  const idx = cache.checkins.findIndex(c => c.id === id);
...
function listCheckinComments(checkinId) {
  assertReady();
  return cache.checkinComments.filter(c => c.checkin_id === checkinId).sort((a, b) => (a.created_at || '').localeCompare(b.created_at || ''));
}
...
function getFriendsPicks(userId, limit = 12) {
  ...
  const friendCheckins = listCheckins({ userIds: friendIds, limit: 100000 }).filter(c => c.rating >= 4 && !ownTried.has(c.strain_id));
  const byStrain = {};
...
module.exports = {
  ...
  createCheckin, listCheckins, getCheckin, updateCheckin, deleteCheckin, giveCheckinKudos,
