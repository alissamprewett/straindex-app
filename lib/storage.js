...
// this covers editing a check-in without changing its photo.
async function uploadCheckinPhoto(dataUrl) {
  ...
  const key = `checkins/${Date.now()}-${crypto.randomBytes(8).toString('hex')}.${ext}`;
...
module.exports = { uploadCheckinPhoto, R2_CONFIGURED };
